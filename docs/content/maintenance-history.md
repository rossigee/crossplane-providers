...
`scripts/audit_standards.sh` + `docs/content/_index.md`, not this file.
...
see `docs/content/standards/platform.md`.
...
## Build-validation + workflow normalization (2026-09-23)

Root cause of red master CI across 17 providers: template's `make docker.build`
target does not exist in `rossigee/build` makelib (`make build` already builds
images via `build.artifacts.platform`). Removed the call from
`docs/templates/ci-template.yml` + all 17 provider `ci.yml` files; aligned to
`make build` + `make xpkg.build` with artifact verification. Special fixes:
libvirt build-validation installs `libvirt-dev`; minio `.gitignore` no longer
hides `package/crds` (Release failed with `cp: cannot stat package/crds` on
2026-09-20 — CRDs force-added, next tag will publish); harbor validate step
used no-op `make build.code`/`make build.artifacts.platform`; backblaze
`security.yml` GO_VERSION 1.26.6→1.27.1 (CodeQL vendor step was failing);
dead `docker.build` Makefile aliases removed from docker + namecheap.
Eventf → `event.Recorder` migrated in backblaze bucket/notification + discord
garbagecollection (crossplane-runtime/v2 API). Stale 2025 headers refreshed on
openstack/backblaze workflows. Audit openstack exclusion dropped (scans 20/20).
18 PRs on `fix/build-validation-2026-09-23`, all merged; 20/20 master CI green;
33 stale `fix/*` branches deleted (all verified merged); meta-repo advanced
(`beac66c`, `ac2252c`).
